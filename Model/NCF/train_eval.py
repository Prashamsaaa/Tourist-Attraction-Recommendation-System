...
def train_model(model, train_loader, test_loader, criterion, optimizer, num_epochs, device, num_items):
    ...
    for epoch in range(num_epochs):
        ...
        for user_ids, item_ids, ratings in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            user_ids, item_ids, ratings = [t.to(device) for t in [user_ids, item_ids, ratings]]
        ...
        hit_rate , ndcg , precision, recall = evaluate_topn(model, test_loader, num_items , top_k = TOP_K,device= DEVICE)
