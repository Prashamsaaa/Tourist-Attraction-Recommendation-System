...
def calculate_ndcg(recommended_items, actual_items, ratings, k):
    ...
    actual_relevance = {item: rating for item, rating in zip(actual_items, ratings)}  # map item to its rating

    dcg = 0.0
    for i, item in enumerate(recommended_items[:k]):
        # Relevance based on rating for the item
        relevance = actual_relevance.get(item, 0)  # default relevance 0 if item not in actual items
        dcg += relevance / np.log2(i + 2)
    
    # Ideal DCG (IDCG) assumes the recommended items are sorted by relevance
    ideal_relevance = sorted(ratings, reverse=True)  # Ideal relevance is highest ratings first
    idcg = 0.0
    for i, item in enumerate(recommended_items[:k]):
