...
class HybridRecommender:
    ...
    def recommend_for_old_user(self, user_id, embeddings, descriptions, ratings, preferred_province, user_encoder, place_encoder):
        ...
        print("recommendation Generate nai bhayena")
        encoded_user_id = user_encoder.transform([user_id])[0]

        ncf_recs = generate_recommendations(self.ncf_recommender, encoded_user_id, num_items= len(place_encoder.classes_),top_k =5, device = 'cpu', place_encoder=place_encoder, attraction_df= filtered_descriptions)
        print(ncf_recs)
