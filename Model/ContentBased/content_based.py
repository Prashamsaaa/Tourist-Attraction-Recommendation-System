...
class ContentBasedRecommender:
    ...
    def create_feature_matrix(self):
        ...
        self.data["Category"] = self.data["Tags"].apply(
            lambda x: [
                category
                for category, tags in self.categorized_tags.items()
                if any(tag in tags for tag in x)
            ]
        )

        self.data["Category"] = self.data["Category"].apply(
            lambda x: ", ".join(x) if x else "Uncategorized"
        )

        province_encoder = LabelEncoder()
        category_encoder = LabelEncoder()
        self.data["Encoded_Province"] = province_encoder.fit_transform(
            self.data["Province"]
        )
        self.data["Encoded_Category"] = category_encoder.fit_transform(
            self.data["Category"]
        )
        ...
        feature_matrix = pd.concat(
            [self.data[["Encoded_Province", "Encoded_Category"]], tag_features_df],
            axis=1,
        )
    ...
    def get_available_tags(self, province=None, categories=None):
        ...
        for category in categories:
            ...
            category_tags = [
                tag for tags_list in category_filtered["Tags"] for tag in tags_list
            ]
        ...
        if len(categories) > 1:
            ...
            combined_tags = [
                tag for tags_list in combined_filtered["Tags"] for tag in tags_list
            ]
    ...
    def recommend(self, province=None, categories=None, tags=None, top_n=5):
        ...
        try:
            ...
            if tags:
                        ...
                        if tag in attraction_tags:
                            ...
                            for at in attraction_tags:
                                ...
                                if tag in at or at in tag:
                                    matches += (
                                        0.8  # Partial match has slightly lower weight
                                    )
                                    break
                ...
                filtered_data = filtered_data[
                    filtered_data["tag_match_score"] > 0
                ].sort_values("tag_match_score", ascending=False)

            # Handle empty results
            if filtered_data.empty:
                print(
                    f"No recommendations found for province={province}, categories={categories}, tags={tags}"
                )
                return pd.DataFrame()

            # Prepare and return results
            result = filtered_data.head(top_n)[
                ["ID", "Name", "Province", "Category", "Tags"]
            ]
