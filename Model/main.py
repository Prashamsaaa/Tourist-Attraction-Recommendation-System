...
def load_data_and_models():
        """
    Comprehensive function to load all necessary data and models for the recommendation system.
    
    Returns:
    - Tuple containing various recommender components, data, and models
    """

    # try:
        # Validate data file existence
        required_files = ['prepared_data', 'tags', 'content', 'ratings']
        for file_key in required_files:
            if not os.path.exists(DATA_PATHS[file_key]):
                raise FileNotFoundError(f"Required file not found: {DATA_PATHS[file_key]}")

        # Content-Based Recommender
        content_recommender = ContentBasedRecommender(
            DATA_PATHS['prepared_data'], 
            DATA_PATHS['tags']
        )
        ...
        backup_path = DATA_PATHS['backup_model']
        ...
        try:
            # Attempt to load from backup or original model path
            logger.info(f"Attempting to load model from: {load_path}")
            checkpoint = torch.load(load_path, map_location=DEVICE)
            
            # Check if it's a full state dict or a partial checkpoint
            if 'model_state_dict' in checkpoint:
                ncf_recommender.load_state_dict(checkpoint['model_state_dict'])
            elif isinstance(checkpoint, dict):
                # Try loading state dict directly
                ncf_recommender.load_state_dict(checkpoint)
            else:
                logger.warning("Checkpoint format not recognized. Initializing new model.")
            ...
            # Optionally, you could add model initialization logic here
        ...
        # Setup Dynamic Model Manager (if needed)
        criterion = torch.nn.MSELoss()
        optimizer = torch.optim.Adam(
            ncf_recommender.parameters(), 
            lr=LEARNING_RATE, 
    ...
    # except Exception as e:
    #     logger.error(f"Critical error in data and model loading: {e}", exc_info=True)
    #     raise

# Note: Ensure all imported modules (NCF.models, NCF.config, etc.) 
