...
from NCF.config import Config
from NCF.preprocess import load_and_preprocess_data, split_data
from NCF.dataset import create_dataloaders
from NCF.NeuMF import NCF
from NCF.train_model import train_epoch, validate
from NCF.recommendation import Recommender
...
def main():
    ...
    ncf_recommender = Recommender(ncf_model_path, item_mapping)
    ...
    # Get available options from content recommender's data
    provinces = content_recommender.data['Province'].unique()
    categories = content_recommender.data['Category'].unique()
    # Flatten the list of tags from all attractions
    all_tags = set()
    for tags_list in content_recommender.data['Tags']:
    ...
    while True:
        ...
        if user_type == "new":
            try:
                ...
                # Get tags specific to the selected category
                ...
                # Print tags in a more readable format, 5 per line
